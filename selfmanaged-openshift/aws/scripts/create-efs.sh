...
FILESYSTEM_ID=`aws efs create-file-system --performance-mode $PERFORMANCE_MODE --tags "Key=Name,Value=$CLUSTERID-efs" --region $REGION --encrypted --query 'FileSystemId' | tr -d '"'`
sleep 30
...
SUBNET_IDS=`aws ec2 describe-subnets --filters "Name=vpc-id,Values=$VPC_ID" "Name=tag:$PRIVATE_SUBNET_TAG_NAME,Values=$PRIVATE_SUBNET_TAG_VALUE" --query 'Subnets[*].SubnetId' --output text`
