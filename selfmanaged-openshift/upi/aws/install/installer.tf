...
resource "null_resource" "openshift_installer" {
  count = var.download_binaries ? 1 : 0
  provisioner "local-exec" {
    command = <<EOF
case $(uname -s) in
  Linux)
    wget -r -l1 -np -nd ${var.openshift_installer_url}/${var.openshift_version} -P ${path.module} -A 'openshift-install-linux-${var.openshift_version}.tar.gz'
    ;;
  Darwin)
    wget -r -l1 -np -nd ${var.openshift_installer_url}/${var.openshift_version} -P ${path.module} -A 'openshift-install-mac-${var.openshift_version}.tar.gz'
    ;;
  *) exit 1
    ;;
esac
...
case $(uname -s) in
  Linux)
    wget -r -l1 -np -nd ${var.openshift_installer_url}/${var.openshift_version} -P ${path.module} -A 'openshift-client-linux-${var.openshift_version}.tar.gz'
    ;;
  Darwin)
    wget -r -l1 -np -nd ${var.openshift_installer_url}/${var.openshift_version} -P ${path.module} -A 'openshift-client-mac-${var.openshift_version}.tar.gz'
    ;;
  *)
    exit 1
    ;;
...
spec:
  ...
  privateZone:
      tags:
        ...
        kubernetes.io/cluster/${local.infrastructure_id}: owned%{if local.public_endpoints}${indent(2, "\npublicZone:")}
    id: ${var.dns_public_id}%{endif}
status: {}
EOF
}
...
resource "local_file" "awssecrets4" {
  count           = var.airgapped.enabled ? 1 : 0
  ...
  file_permission = "0644"
  filename        =  "${path.module}/temp/openshift/99_awssecrets_ebs_cloud.yml"
