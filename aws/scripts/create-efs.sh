...
EFS_SG_GROUPID=`aws ec2 create-security-group --group-name EFSSecutityGroup --description "EFS security group" --vpc-id $VPD_ID | awk -F':' '{print $2}' | awk '{print $1}' | xargs | tr -d '"'`
aws ec2 authorize-security-group-ingress --group-id $EFS_SG_GROUPID --protocol tcp --port 2049 --cidr 10.0.0.0/16
aws ec2 authorize-security-group-ingress --group-id $EFS_SG_GROUPID --protocol tcp --port 22 --cidr 0.0.0.0/0
...
SUBNET_IDS=`aws ec2 describe-subnets --filters "Name=vpc-id,Values=$VPD_ID" "Name=tag:Name,Values=*private*" --query 'Subnets[*].SubnetId' --output text`
