...
resource "null_resource" "retrieve_ibm_cp_datacore" {
  provisioner "local-exec" {
    interpreter = ["/bin/bash", "-c"]
    command = "rm -rf ibm-cp-datacore* && wget --no-verbose https://github.com/IBM/cloud-pak/raw/master/repo/case/ibm-cp-datacore/1.3.2/ibm-cp-datacore-1.3.2.tgz && tar -xf ibm-cp-datacore-*.tgz"
  }
  provisioner "local-exec" {
    when = destroy
    interpreter = ["/bin/bash", "-c"]
