...
then
    ...
    plusFourHoursUtc=$(date -u -v+8H +%Y-%m-%dT%H:%MZ 2>/dev/null)  || plusFourHoursUtc=$(date -u --date "$dte 8 hour" +%Y-%m-%dT%H:%MZ)
...
then
    if [[ $uploadArtifacts || $_artifactsLocationParameter != null ]]
    then
        az group deployment validate -g "$resourceGroupName" --template-uri $templateUri --parameters "$parameterJson"
    else
        az group deployment validate -g "$resourceGroupName" --template-file $templateFile --parameters "$parameterJson"
    fi
else
    if [[ $uploadArtifacts || $_artifactsLocationParameter != null ]]
    then
        az group deployment create -g "$resourceGroupName" -n AzureRMSamples --template-uri $templateUri --parameters "$parameterJson"
    else
        az group deployment create -g "$resourceGroupName" -n AzureRMSamples --template-file $templateFile --parameters "$parameterJson"
    fi
fi
