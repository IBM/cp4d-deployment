...
then
    ...
    if [[ $uploadArtifacts || $_artifactsLocationParameter != null ]]
    then
        az deployment group create -g "$resourceGroupName" -n CPD --template-uri $templateUri --parameters "$parameterJson" --verbose
    else
        az deployment group create -g "$resourceGroupName" -n CPD --template-file $templateFile --parameters "$parameterJson" --verbose
    fi
fi
