...
## WKC Catalog Source and Subscription
echo "Deploying catalogsources and operator subscriptions for  watson knowledge catalog"
runuser -l $SUDOUSER -c "sudo $CPDTEMPLATES/cpd-cli manage apply-olm --release=${VERSION} --components=wkc"

if [ $? -ne 0 ]
then
    echo "**********************************"
    echo "Deploying catalog Sources & subscription failed for WKC"
    echo "**********************************"
    exit 1
fi
