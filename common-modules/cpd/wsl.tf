...
echo "Create ws cr" &&
bash cpd/scripts/apply-cr.sh ${self.triggers.cpd_workspace} ${var.cpd_version} ws ${var.cpd_namespace} ${var.storage_option} ${local.storage_class} ${local.rwo_storage_class}
EOF
  }
  depends_on = [
